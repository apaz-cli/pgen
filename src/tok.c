...
typedef void (*pcc_action_t)(tok_context_t *, pcc_thunk_t *, pcc_value_t *);
...
typedef pcc_thunk_chunk_t *(*pcc_rule_t)(tok_context_t *);
...
static tok_context_t *pcc_context__create(pcc_auxil_t auxil) {
    tok_context_t *const ctx = (tok_context_t *)PCC_MALLOC(auxil, sizeof(tok_context_t));
    ctx->pos = 0;
    ctx->cur = 0;
    ctx->level = 0;
...
static void pcc_context__destroy(tok_context_t *ctx) {
    if (ctx == NULL) return;
...
static size_t pcc_refill_buffer(tok_context_t *ctx, size_t num) {
...
MARK_USED_FUNC
static void pcc_commit_buffer(tok_context_t *ctx) {
...
MARK_USED_FUNC
static const char *pcc_get_capture_string(tok_context_t *ctx, const pcc_capture_t *capt) {
    if (capt->string == NULL)
...
static size_t pcc_get_char_as_utf32(tok_context_t *ctx, int *out) { /* with checking UTF-8 validity */
    int c, u;
    size_t n;
...
MARK_USED_FUNC
static pcc_bool_t pcc_apply_rule(tok_context_t *ctx, pcc_rule_t rule, pcc_thunk_array_t *thunks, pcc_value_t *value) {
...
MARK_USED_FUNC
static void pcc_do_action(tok_context_t *ctx, const pcc_thunk_array_t *thunks, pcc_value_t *value) {
    size_t i;
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_TokenFile(tok_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_WS(tok_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_ML_COMMENT(tok_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_SL_COMMENT(tok_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule__(tok_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_num(tok_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_tokident(tok_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_char(tok_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_numset(tok_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_charset(tok_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_pair(tok_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_TokenDef(tok_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_LitDef(tok_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_SMDef(tok_context_t *ctx);

static pcc_thunk_chunk_t *pcc_evaluate_rule_TokenFile(tok_context_t *ctx) {
    pcc_thunk_chunk_t *const chunk = pcc_thunk_chunk__create(ctx->auxil);
    chunk->pos = ctx->cur;
    PCC_DEBUG(ctx->auxil, PCC_DBG_EVALUATE, "TokenFile", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->buffer.len - chunk->pos));
    ctx->level++;
    ...
    pcc_capture_table__resize(ctx->auxil, &chunk->capts, 0);
    {
        ...
        for (i = 0;; i++) {
            ...
            const size_t n = chunk->thunks.len;
            {
                ...
                if (!pcc_apply_rule(ctx, pcc_evaluate_rule_TokenDef, &chunk->thunks, NULL)) goto L0003;
                if (!pcc_apply_rule(ctx, pcc_evaluate_rule__, &chunk->thunks, NULL)) goto L0003;
                goto L0002;
            L0003:;
                ctx->cur = p;
    ...
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_MATCH, "TokenFile", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    return chunk;
L0000:;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_NOMATCH, "TokenFile", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_ML_COMMENT(tok_context_t *ctx) {
    pcc_thunk_chunk_t *const chunk = pcc_thunk_chunk__create(ctx->auxil);
    chunk->pos = ctx->cur;
    PCC_DEBUG(ctx->auxil, PCC_DBG_EVALUATE, "ML_COMMENT", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->buffer.len - chunk->pos));
    ctx->level++;
    pcc_value_table__resize(ctx->auxil, &chunk->values, 0);
    pcc_capture_table__resize(ctx->auxil, &chunk->capts, 0);
    if (
    ...
    ctx->cur += 2;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_MATCH, "ML_COMMENT", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    return chunk;
L0000:;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_NOMATCH, "ML_COMMENT", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    pcc_thunk_chunk__destroy(ctx->auxil, chunk);
    return NULL;
}
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_SL_COMMENT(tok_context_t *ctx) {
    pcc_thunk_chunk_t *const chunk = pcc_thunk_chunk__create(ctx->auxil);
    chunk->pos = ctx->cur;
    PCC_DEBUG(ctx->auxil, PCC_DBG_EVALUATE, "SL_COMMENT", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->buffer.len - chunk->pos));
    ctx->level++;
    pcc_value_table__resize(ctx->auxil, &chunk->values, 0);
    pcc_capture_table__resize(ctx->auxil, &chunk->capts, 0);
    if (
    ...
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_MATCH, "SL_COMMENT", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    return chunk;
L0000:;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_NOMATCH, "SL_COMMENT", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    pcc_thunk_chunk__destroy(ctx->auxil, chunk);
    return NULL;
}
    ...
    pcc_capture_table__resize(ctx->auxil, &chunk->capts, 0);
    {
        ...
        for (i = 0;; i++) {
            ...
            L0003:;
                ctx->cur = p;
                pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
                if (!pcc_apply_rule(ctx, pcc_evaluate_rule_ML_COMMENT, &chunk->thunks, NULL)) goto L0004;
                goto L0002;
            L0004:;
                ctx->cur = p;
                pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
                if (!pcc_apply_rule(ctx, pcc_evaluate_rule_SL_COMMENT, &chunk->thunks, NULL)) goto L0005;
                goto L0002;
            L0005:;
                ctx->cur = p;
                pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_numset(tok_context_t *ctx) {
    pcc_thunk_chunk_t *const chunk = pcc_thunk_chunk__create(ctx->auxil);
    chunk->pos = ctx->cur;
    PCC_DEBUG(ctx->auxil, PCC_DBG_EVALUATE, "numset", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->buffer.len - chunk->pos));
    ctx->level++;
    pcc_value_table__resize(ctx->auxil, &chunk->values, 0);
    pcc_capture_table__resize(ctx->auxil, &chunk->capts, 0);
    {
        const size_t p = ctx->cur;
        const size_t n = chunk->thunks.len;
        if (!pcc_apply_rule(ctx, pcc_evaluate_rule_num, &chunk->thunks, NULL)) goto L0002;
        goto L0001;
    L0002:;
        ctx->cur = p;
        pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
    ...
    L0003:;
        ...
        if (!pcc_apply_rule(ctx, pcc_evaluate_rule__, &chunk->thunks, NULL)) goto L0004;
        {
            ...
            for (i = 0;; i++) {
                const size_t p = ctx->cur;
                const size_t n = chunk->thunks.len;
                if (!pcc_apply_rule(ctx, pcc_evaluate_rule_numset, &chunk->thunks, NULL)) goto L0005;
                if (
                    pcc_refill_buffer(ctx, 1) < 1 ||
                    ctx->buffer.buf[ctx->cur] != ','
                ) goto L0005;
        ...
        if (!pcc_apply_rule(ctx, pcc_evaluate_rule_numset, &chunk->thunks, NULL)) goto L0004;
        if (!pcc_apply_rule(ctx, pcc_evaluate_rule__, &chunk->thunks, NULL)) goto L0004;
        if (
            pcc_refill_buffer(ctx, 1) < 1 ||
            ctx->buffer.buf[ctx->cur] != ')'
    ...
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_MATCH, "numset", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    return chunk;
L0000:;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_NOMATCH, "numset", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    pcc_thunk_chunk__destroy(ctx->auxil, chunk);
    return NULL;
}
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_charset(tok_context_t *ctx) {
    pcc_thunk_chunk_t *const chunk = pcc_thunk_chunk__create(ctx->auxil);
    chunk->pos = ctx->cur;
    PCC_DEBUG(ctx->auxil, PCC_DBG_EVALUATE, "charset", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->buffer.len - chunk->pos));
    ctx->level++;
    pcc_value_table__resize(ctx->auxil, &chunk->values, 0);
    pcc_capture_table__resize(ctx->auxil, &chunk->capts, 0);
    {
        ...
        }
        {
            ...
            for (i = 0;; i++) {
                ...
                const size_t n = chunk->thunks.len;
                {
                    ...
                    if (!pcc_apply_rule(ctx, pcc_evaluate_rule_char, &chunk->thunks, NULL)) goto L0008;
                    {
                        const size_t p = ctx->cur;
                        if (
                            pcc_refill_buffer(ctx, 1) < 1 ||
                ...
                L0008:;
                    ...
                    if (!pcc_apply_rule(ctx, pcc_evaluate_rule_char, &chunk->thunks, NULL)) goto L0012;
                    goto L0007;
                L0012:;
                    ctx->cur = p;
                    pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
    ...
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_MATCH, "charset", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    return chunk;
L0000:;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_NOMATCH, "charset", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    pcc_thunk_chunk__destroy(ctx->auxil, chunk);
    return NULL;
}
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_pair(tok_context_t *ctx) {
    ...
    ctx->cur++;
    if (!pcc_apply_rule(ctx, pcc_evaluate_rule_numset, &chunk->thunks, NULL)) goto L0000;
    if (
        pcc_refill_buffer(ctx, 2) < 2 ||
        (ctx->buffer.buf + ctx->cur)[0] != ',' ||
        (ctx->buffer.buf + ctx->cur)[1] != ' '
    ...
    ctx->cur += 2;
    if (!pcc_apply_rule(ctx, pcc_evaluate_rule_charset, &chunk->thunks, NULL)) goto L0000;
    if (
        pcc_refill_buffer(ctx, 1) < 1 ||
        ctx->buffer.buf[ctx->cur] != ')'
    ) goto L0000;
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_TokenDef(tok_context_t *ctx) {
    pcc_thunk_chunk_t *const chunk = pcc_thunk_chunk__create(ctx->auxil);
    chunk->pos = ctx->cur;
    PCC_DEBUG(ctx->auxil, PCC_DBG_EVALUATE, "TokenDef", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->buffer.len - chunk->pos));
    ctx->level++;
    pcc_value_table__resize(ctx->auxil, &chunk->values, 0);
    pcc_capture_table__resize(ctx->auxil, &chunk->capts, 0);
    if (!pcc_apply_rule(ctx, pcc_evaluate_rule_tokident, &chunk->thunks, NULL)) goto L0000;
    if (
        pcc_refill_buffer(ctx, 1) < 1 ||
        ctx->buffer.buf[ctx->cur] != ':'
    ) goto L0000;
    ...
    if (!pcc_apply_rule(ctx, pcc_evaluate_rule__, &chunk->thunks, NULL)) goto L0000;
    {
        const size_t p = ctx->cur;
        const size_t n = chunk->thunks.len;
        if (!pcc_apply_rule(ctx, pcc_evaluate_rule_LitDef, &chunk->thunks, NULL)) goto L0002;
        goto L0001;
    L0002:;
        ctx->cur = p;
        pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
        if (!pcc_apply_rule(ctx, pcc_evaluate_rule_SMDef, &chunk->thunks, NULL)) goto L0003;
        goto L0001;
    L0003:;
        ctx->cur = p;
        pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
    ...
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_MATCH, "TokenDef", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    return chunk;
L0000:;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_NOMATCH, "TokenDef", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    pcc_thunk_chunk__destroy(ctx->auxil, chunk);
    return NULL;
}
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_LitDef(tok_context_t *ctx) {
    pcc_thunk_chunk_t *const chunk = pcc_thunk_chunk__create(ctx->auxil);
    chunk->pos = ctx->cur;
    PCC_DEBUG(ctx->auxil, PCC_DBG_EVALUATE, "LitDef", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->buffer.len - chunk->pos));
    ctx->level++;
    pcc_value_table__resize(ctx->auxil, &chunk->values, 0);
    pcc_capture_table__resize(ctx->auxil, &chunk->capts, 0);
    if (
    ...
    ctx->cur += 2;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_MATCH, "LitDef", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    return chunk;
L0000:;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_NOMATCH, "LitDef", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    pcc_thunk_chunk__destroy(ctx->auxil, chunk);
    return NULL;
}
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_SMDef(tok_context_t *ctx) {
    pcc_thunk_chunk_t *const chunk = pcc_thunk_chunk__create(ctx->auxil);
    chunk->pos = ctx->cur;
    PCC_DEBUG(ctx->auxil, PCC_DBG_EVALUATE, "SMDef", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->buffer.len - chunk->pos));
    ctx->level++;
    pcc_value_table__resize(ctx->auxil, &chunk->values, 0);
    pcc_capture_table__resize(ctx->auxil, &chunk->capts, 0);
    if (!pcc_apply_rule(ctx, pcc_evaluate_rule_numset, &chunk->thunks, NULL)) goto L0000;
    ...
    ctx->cur++;
    {
        ...
        for (i = 0;; i++) {
            ...
            ctx->cur += 2;
            if (!pcc_apply_rule(ctx, pcc_evaluate_rule__, &chunk->thunks, NULL)) goto L0001;
            if (!pcc_apply_rule(ctx, pcc_evaluate_rule_num, &chunk->thunks, NULL)) goto L0001;
            {
                const size_t p = ctx->cur;
                const size_t n = chunk->thunks.len;
                if (
    ...
    ctx->cur += 2;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_MATCH, "SMDef", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    return chunk;
L0000:;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_NOMATCH, "SMDef", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    pcc_thunk_chunk__destroy(ctx->auxil, chunk);
    return NULL;
}
...
int tok_parse(tok_context_t *ctx, ASTNode**ret) {
    if (pcc_apply_rule(ctx, pcc_evaluate_rule_TokenFile, &ctx->thunks, ret))
